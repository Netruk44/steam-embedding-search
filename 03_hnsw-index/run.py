...
@click.option('--index-type', type=click.Choice(['description', 'review', 'all']), default='all', help='Type of index to create')
...
def get_reviews_by_appid_batched(conn: sqlite3.Connection, page_size: int = 1000) -> Iterator[List[Tuple[int, List[float]]]]:
  appids = sqlite_helpers.get_appids_with_reviews(conn)
